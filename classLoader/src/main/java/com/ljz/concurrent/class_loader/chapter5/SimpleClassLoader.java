...
public class SimpleClassLoader extends ClassLoader {
    private final static String DEFAULT_DIR = "D:\\work\\revert";
    ...
    protected Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        //查找并加载类
        try {
            clazz = findClass(name);//自己先去找
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
        if (null == clazz && getParent() != null) { //如果没找到且有父加载器
            getParent().loadClass(name); //让父加载器去找
        }
