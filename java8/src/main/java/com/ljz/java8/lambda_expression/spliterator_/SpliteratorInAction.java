package com.ljz.java8.lambda_expression.spliterator_;
 ...
 * @Project: java8
 * @Package: com.ljz.java8.lambda_expression.spliterator_
 * @Description: 所有的Stream里都有内置的 Spliterator ，Spliterator底层使用了fork join
 * @author: LiuJiaZe
 * @date: 2019年12月02日 14:04
 * @version: V1.0
 */
public class SpliteratorInAction {
    ...
    public static void main(String[] args) {
        ...
        //        MyUtils.getMethds(Spliterator.class);
        /**
         * forEachRemaining： 不断执行tryAdvance 直到条件为false为止
         * characteristics：
         * estimateSize：
         * getComparator：
         * tryAdvance：不管stream是否并行，都会调用，试图从流里拿元素消费，如果有执行Consumer，没有返回false
         * trySplit：对应分配任务
         * getExactSizeIfKnown：获取剩余元素
         * hasCharacteristics：
         */
        //        demonstration();


        //将字符串按 "\n"分隔，获取String[] ，用封装的Stream 按行读取
        MySpliteratorText mySpliteratorText = new MySpliteratorText(text);
        //拿出stream 非并行只会调用tryAdvance 不会调用trySplit
        ...
        mySpliteratorText.stream().filter(s -> !s.equals("")).forEach(System.out::println);//去掉回车

        System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");


        //使用并行stream 会调用trySplit
        Optional.ofNullable(mySpliteratorText.parallelStream().count()).ifPresent(System.out::println);
        //        mySpliteratorText.stream().forEach(System.out::println);
        mySpliteratorText.parallelStream().filter(s -> !s.equals("")).forEach(System.out::println);//去掉回车


        //思考
        IntStream intStream = IntStream.rangeClosed(0, 100);
        //parallel() ， sequential() 只是设置一个是否并行的标识 ，以最后一个为主
        intStream.parallel().filter(null).sequential().map(null).parallel().forEach(null);
    ...
    static class MySpliteratorText {
        ...
        public Stream<String> stream() {
            return StreamSupport.stream(new MySpliterator(), false); //false 不是并行的
        }

        public Stream<String> parallelStream() {
            return StreamSupport.stream(new MySpliterator(), true); //true 并行
        }
        ...
        private class MySpliterator implements Spliterator<String> {
            ...
            public boolean tryAdvance(Consumer<? super String> action) {
                if (start <= end) {
                    action.accept(MySpliteratorText.this.data[start++]);//start++; 前进一位
                    return true;
                }
            ...
            public int characteristics() {
                return IMMUTABLE & SIZED & SUBSIZED; //不可变 大小固定，可取子集
            }
